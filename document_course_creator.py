...
async def generate_content_with_mistral(prompt, response_schema=None, temperature=0.7, max_tokens=2048, top_k=32, top_p=1.0):
        ...
        if result.get("choices") and result["choices"][0].get("message"):
            ...
            if response_schema:
                try:
                    ...
                    from course_generator import safe_json_parse
...
def init_session_state():
    ...
    if "doc_conversation" not in st.session_state:
        llm = ChatMistralAI(api_key=MISTRAL_API_KEY, model="mistral-large-latest", temperature=0.7)
...
def run_app():
        ...
        if uploaded_files:
            with st.spinner("Processing documents and generating course outline..."):
                ...
                course_data = loop.run_until_complete(
                    generate_content_with_mistral(course_prompt, response_schema=course_schema)
                )
    ...
    if st.session_state.selected_doc_course is not None and st.session_state.doc_courses:
        ...
        for m_idx, module in enumerate(selected_course.get("modules", [])):
            ...
            for c_idx, chapter in enumerate(module.get("chapters", [])):
                ...
                if st.button(f"Generate Detailed Content for '{chapter['chapterTitle']}'", key=f"doc_gen_content_{chapter_id}"):
                    with st.spinner("Generating detailed chapter content..."):
                        ...
                        detailed_content = loop.run_until_complete(
                            generate_content_with_mistral(content_prompt, temperature=0.7, max_tokens=2048, top_k=32, top_p=1.0)
                        )
                ...
                if chapter_id in st.session_state.chapter_contents:
                    ...
                    st.markdown("> *Click 'Generate Detailed Content' to get more information for this chapter.*")
            ...
            if st.button(f"Take Quiz for Module {module['moduleNumber']} ({module['moduleTitle']})", key=f"doc_quiz_btn_{module_quiz_id}"):
                with st.spinner("Generating quiz... This may take a moment."):
                    ...
                    if not module_content.strip():
                        ...
                        if not MISTRAL_API_KEY:
                            st.error("🔑 Mistral API Key is required for quiz generation. Please configure your Mistral API key in Streamlit secrets.")
                            ...
                            quiz_data = loop.run_until_complete(quiz_utils.generate_quiz_with_mistral(
                                module_content, MISTRAL_API_KEY, 0.5, 2048, 1, 1, num_questions=5 # Using default quiz params for now
                            ))
                            loop.close() # Close the loop after use
        ...
        st.markdown("> 📥 **Please upload documents and generate a new course to get started!**")
    elif st.session_state.selected_doc_course is None and st.session_state.doc_courses:
        st.markdown("> 👈 **Select an existing document-generated course from the sidebar to view its content.**")
